...
echo  "$($PSQL "SELECT DISTINCT name FROM games INNER JOIN teams ON games.winner_id = teams.team_id WHERE year = 2014 UNION DISTINCT SELECT DISTINCT name FROM games INNER JOIN teams ON games.opponent_id=teams.team_id WHERE year=2014 ORDER BY name")"
